...
# GCCPLUGINS_DIR and GCCPLUGINS_API_VERSION can be overwritten by the user
# and provide basic support for x-compiling between gcc versions,
# eg. using your standard x86 distribution gcc 4.7 to compile a plugin
# for your x-gcc 4.9 for another target
# You can use:
#    ~# make plugin \
#            GCCPLUGINS_DIR=$(<YOUR/X-GCC/BIN/PATH> --print-file-name=plugin) \
#            GCCPLUGINS_API_VERSION=<X-GCC VERSION>
# where <YOUR/X-GCC/BIN/PATH> is the path to your x-gcc binary
# and <X-GCC VERSION> is the version of your x-gcc version with the format
# major * 1000 + minor, eg. 4.9 = 4009
# it defaults to the GCC internal GCC_VERSION #define
GCCPLUGINS_DIR= $(shell $(CC) --print-file-name=plugin)
GCCPLUGINS_API_VERSION=GCC_VERSION
...
CPPFLAGS+= -I$(GCCPLUGINS_DIR)/include -I$(GCCPLUGINS_DIR)/include/c-family -I. $(PYTHON_INCLUDES) -DGCCPLUGINS_API_VERSION=$(GCCPLUGINS_API_VERSION)
# Allow user to pick optimization, choose whether warnings are fatal,
# and choose debugging information level.
CFLAGS?=-O2 -Werror -g
# Force these settings
override CFLAGS:=$(CFLAGS) -fPIC -fno-strict-aliasing -Wall
LIBS+= $(PYTHON_LIBS)
ifneq "$(PLUGIN_PYTHONPATH)" ""
  CPPFLAGS+= -DPLUGIN_PYTHONPATH='"$(PLUGIN_PYTHONPATH)"'
endif

export CC CFLAGS GCCPLUGINS_DIR GCCPLUGINS_API_VERSION
...
$(LIBGCC_C_API_SO):
	$(MAKE) -C gcc-c-api libgcc-c-api.so
